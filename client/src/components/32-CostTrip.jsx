...
export default function CostTrip({distance, unicFuel}){
    ...
    return (
        <div>
            ...
            {<>
            <select onChange={(e)=>setL( {...l, int: parseInt(e.target.value)} )}>
                {numbers.slice(0,20).map((n, i)=><option key={i}>{n}</option>)}
            </select>
            <select onChange={(e)=>setL( {...l, dec: parseInt( e.target.value.slice(1))*0.1} )}>
                {numbers.slice(0,10).map((n, i)=><option key={i}>.{n}</option>)}
            </select>
            </>}
            {litres && <>
            <b style={{"color": "blue"}}> {total}</b> litros por cada 100 km.
            </>}
            {!litres && <>
            <b style={{"color": "magenta"}}> {total}</b> kilómetros por litro.
            </>
            }
            <br/>
            ...
            <button onClick={execute} disabled={disable()}>
                Estimar precio de un viaje
            </button>
            {result&& <><br/>Precio estimado: ${parseFloat(result.toFixed(2))}</>}
        </div>
    )
}